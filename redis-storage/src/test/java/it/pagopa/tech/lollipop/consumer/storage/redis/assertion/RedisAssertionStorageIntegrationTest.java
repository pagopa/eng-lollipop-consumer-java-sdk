...
class RedisAssertionStorageIntegrationTest {
    ...
    private AssertionStorage redisStorage;
    ...
    public void setUp() throws IOException {
        ...
        storageConfig.setStorageEvictionDelay(1);
        storageConfig.setStorageEvictionDelayTimeUnit(TimeUnit.SECONDS);
        redisServer = RedisServer.newRedisServer().build();
        redisSentinel = RedisSentinel.newRedisSentinel().build();
        redisServer.start();
        redisSentinel.start();
        redisStorage =
                new RedisAssertionStorageProvider(
    ...
    public void successfulOperationsOnRedisStorage() {
        ...
        samlAssertion.setAssertionRef("test-key");
        samlAssertion.setAssertionData(VALID_ASSERTION_XML);
        redisStorage.saveAssertion("test-key", samlAssertion);
        Thread.sleep(150);
        SamlAssertion result = redisStorage.getAssertion("test-key");
        Assertions.assertEquals(samlAssertion, result);
    }
